...
public abstract class EntityTitan extends EntityLiving implements IEntityMultiPart {
    ...
    public static final IAttribute STAMINA = new RangedAttribute(null, "titan.stamina", 2000.0D, 0.0D, Double.MAX_VALUE).setDescription("Stamina").setShouldWatch(true);

    public static final IAttribute MAX_TITAN_HEALTH = new RangedAttribute(null, "titan.maxHealth", 2000.0D, Float.MIN_VALUE, Float.MAX_VALUE).setDescription("maxHealth").setShouldWatch(true);

    private static final DataParameter<Float> TITAN_HEALTH = EntityDataManager.<Float>createKey(EntityTitan.class, DataSerializers.FLOAT);
...
//		getAttributeMap().registerAttribute(HEALTH);
        ...
        getAttributeMap().registerAttribute(MAX_TITAN_HEALTH);
        getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(getMaxHealth());
    ...
    protected void entityInit() {
        ...
        this.dataManager.register(TITAN_HEALTH, 1000.0F);
    }

    public void readEntityFromNBT(NBTTagCompound tagCompund) {
        super.readEntityFromNBT(tagCompund);
        if (tagCompund.hasKey("healthTitan")) {
            setHealth(tagCompund.getFloat("healthTitan"));
    ...
    public void writeEntityToNBT(NBTTagCompound tagCompound) {
        tagCompound.setFloat("healthTitan", getHealth());
    ...
    public void onUpdate() {
        ...
        if (this instanceof EntityPreTitan)
            return;
    ...
    public void onDeath(DamageSource source) {
        ...
        if (entity != null && entity instanceof EntityLivingBase) {
            ...
            if (!world.isRemote) {
                ...
                int i = EnchantmentHelper.getLootingModifier(killer);
...
//	public double getTrueMaxHealth(){
//		return this.getEntityAttribute(MAX_TITAN_HEALTH).getAttributeValue();
//	};
...
//	{
//		return ((double) this.dataManager.get(TITAN_HEALTH));
//	}

    @Override
    public float getHealth() {
        return this.dataManager.get(TITAN_HEALTH);
    }

    @Override
    public void setHealth(float health) {
        this.dataManager.set(TITAN_HEALTH, MathHelper.clamp(health, 0.0F, getMaxHealth()));
    }

//	protected void setHealth(double value)
//	{
//		this.dataManager.set(TITAN_HEALTH, ((float) MathHelper.clamp(value, 0.0, getMaxHealth())));
//		this.getEntityAttribute(HEALTH).setBaseValue(MathHelper.clamp(value, 0.0D, maxHealth));
//
//		if (maxHealth < health)
...
//	}
    ...
    public float getMaxHealth() {
        return (float) this.getEntityAttribute(MAX_TITAN_HEALTH).getAttributeValue();
    }
