...
public abstract class EntityPreTitan extends EntityTitan implements IRangedAttackMob, IBossBar, IMob {
    ...
    public void ignite() {
        ...
        playSound(SoundRegistry.get("titan.birth"), 10F, 1F);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.getInvulTime() > 0 && this.deathTicks <= 0) {
            ...
            this.heal(0.001F * this.getSizeMultiplier());
            ...
            if (!world.isRemote) {
                ...
                if (deathTicks <= 0)
                    ...
                    {
                        if (entity.height <= this.height * 0.5 && !entity.noClip && !(entity instanceof EntityFireball) && entity.isNonBoss() && entity.getEntityBoundingBox().intersects(getEntityBoundingBox())) {
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTicks == 100)
            for (EntityPlayer entityplayer : world.playerEntities)
                world.playSound(entityplayer, getPosition(), SoundRegistry.get("titan.fall"), this.getSoundCategory(), getSoundVolume(), getSizeMultiplier() >= 23 ? 0.75F : 1F);
